...
- Každý tým má **základnu rozloženou podél okraje** hřiště (edge spread).
  - Roboti: levý okraj (`q = -max`).
  - Mutanti: pravý okraj (`q = +max`).
- Počet base hexů = `unitsPerTeam`, rovnoměrně rozmístěných podél okraje.
...
- Každá jednotka má unikátní číslo (**1-based**: 1-4) zobrazené nad hlavou.
...
### Zdi (Robot)
- Stavba na **sousední prázdný hex** (ne base, ne obsazený) — **expansive build**: robot může stavět i na neutrální/nepřátelský hex (automaticky ho zabere).
- Cena: **2 energie**.
...
- Build (Robot): blokováno pokud sousední hex není prázdný non-base (expansive: neutral/enemy OK), nebo nedostatek energie, nebo dosažen wall cap.
...
- Neúplné soubory (přerušená hra) se **automaticky mažou** (OnDestroy flush při opuštění Play mode).
- Latest replay tlačítko **přeskakuje** soubory < 100 bytů (neúplné).
...
- Hráč ovládá jeden tým (Robot nebo Mutant), AI ovládá druhý tým pomocí natrénovaného ONNX modelu.
- Spuštění: Project Tools → "Play as Robots" / "Play as Mutants".
- **HumanTurnController** nahrazuje HexAgent pro lidský tým (stejný turn protocol).
- **HumanInputManager**: klikání na hex pro výběr cíle, klávesy 1-4 / M/A/B/D pro akce.
- **Toolbar:** tlačítka Move, Attack, Build, Destroy, Idle — nevalidní akce jsou **disabled** (šedé).
- **HexHighlighter:** ukazuje platné cíle barevně (zelená=move, červená=attack, modrá=build, žlutá=destroy). Pool 7 cylindrů.
- **Active unit glow:** pulsující emission na modelu aktivní jednotky (modrá pro roboty, zelená pro mutanty).
- **AI delay:** 0.5s pauza po AI tahu pro čitelnost.
- **Turn log:** posledních 10 tahů s zarovnanými sloupci (#, Unit, Action), 1-based numbering.
- **Thinking stats:** aktuální a průměrný čas na rozhodnutí.
- **Respawn cooldown:** mrtvé jednotky zobrazují ⏳ + počet kroků místo energie.
- **DisplayName:** vlastní jména jednotek, bez podtržítek v UI (FormatUnitName).
- **Match history:** sloupec Time (M:SS) zobrazující délku zápasu.
- TimeScale = 1 (reálný čas).
